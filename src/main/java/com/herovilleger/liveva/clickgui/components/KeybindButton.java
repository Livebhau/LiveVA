...
public class KeybindButton extends ButtonComponent {
    ...
    private final List<Integer> keybindBlacklist = List.of(GLFW.GLFW_KEY_UNKNOWN, GLFW.GLFW_MOUSE_BUTTON_LEFT, GLFW.GLFW_KEY_ESCAPE);
    public MutableText unbound  = Text.literal("Not Bound").withColor(0xffffff);
    public MutableText binding  = Text.literal("Press Key...").withColor(0xffffff);
    public boolean isBinding    = false;

    public KeybindButton() {
        super(Text.empty(), button -> {});
        this.onPress(button -> {
            if (this.isBinding) this.bind(GLFW.GLFW_KEY_UNKNOWN);
            else { this.setMessage(this.binding); this.isBinding = true; }
        });
        this.renderer((context, btn, delta) -> {
            context.fill(btn.getX(), btn.getY(), btn.getX() + btn.getWidth(), btn.getY() + btn.getHeight(), 0xff101010);
            context.drawRectOutline(btn.getX(), btn.getY(), btn.getWidth(), btn.getHeight(), 0xff5ca0bf);
        });
    ...
    public Text getKeyLabel(int keycode) {
        ...
        if (input.getLocalizedText().getString().equals(input.getTranslationKey())) {
    ...
    public EventSource<KeybindChanged> onBound() { return changedEvents.source(); }

    private boolean valid(int key) {
        return !keybindBlacklist.contains(key);
    }

    public interface KeybindChanged {
        static EventStream<KeybindChanged> newStream() {
            return new EventStream<>(subscribers -> (keycode) -> {
                for (var subscriber : subscribers) subscriber.onBind(keycode);
            });
        }
