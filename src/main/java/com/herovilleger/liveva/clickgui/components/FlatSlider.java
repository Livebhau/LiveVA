...
public class FlatSlider extends SlimSliderComponent {
    ...
    public FlatSlider(int trackColor, int sliderColor) {
        ...
        this.keyPress().subscribe((click) -> {
            if (click.key() == GLFW.GLFW_KEY_LEFT)  { this.value(this.value() - this.stepSize); return true; }
            if (click.key() == GLFW.GLFW_KEY_RIGHT) { this.value(this.value() + this.stepSize); return true; }
            return false;
        });
    }
